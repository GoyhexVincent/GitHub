...
proxy = req.ProxyHandler({'http':'proxy.brgm.fr:8080' })
#http://www.4imn.com/top200/# Lien utile vers les 200 plus gros journaux au monde.


#d = fp.parse('http://www.reddit.com/r/python/.rss')
d = fp.parse ('http://www.theguardian.com/world/rss', handlers = [proxy])
#d = fp.parse('http://rss.nytimes.com/services/xml/rss/nyt/World.xml')
...
for line in feeds_to_parse:
    print (line)
    print ('\a'*60)
    parser = fp.parse(str(line),handlers = [proxy])
